...
## Calculating cumulative probabilities for any truncated continuous distribution
...
This will look a bit more involved, but it's the same basic concept applied to calculating the inverse cumulative probability. Assuming we are working with a continuous distribution, we start with:
...
### Special considerations for discrete distributions

Recall that for continuous distributions $Pr\{X \leq x\} = Pr\{X < x\}$. This allowed us to ignore the lower bound in the calculations we should above for continuous distributions. However, with discrete distributions, we are no longer able to ignore the lower bound, and must explicitly account for it in our calculations. 

Using "Pr" notation, we show the general calculations for $F(x)$ and $F^{-1}(\alpha)$ for a discrete truncated distribution:

$$F(x) = \frac{Pr\{X\leq x\}-Pr\{X < a\}}{Pr\{X\leq b\}-Pr\{X < a\}}$$

$$x = F^{-1}\big(\alpha\big[Pr\{X\leq b\}-Pr\{X < a\}\big] + Pr\{X < a\}\big)$$

Now let's say we have a random variable $x \sim Poisson(\lambda)$ with bounds $[a, b]$. We know the Poisson is a non-negative integer distribution. That means there is *no probability mass between integer valued outcomes*, i.e. the probability of any $x \in (0,1)=0$; recall that when specifying intervals, rounded parentheses $()$ indicate the corresponding bound is *excluded* while brackets $[]$ indicate the corresponding bound is *includeed*. Calculating $F_{a,b}(x)$ as we did above for the continuous distribution, i.e., using $F_{a,b}(x)=\frac{F(x)-F(a)}{F(b)-F(a)}$, would be incorrect, because $F(a)$ includes the probability mass for $a$. We need to make the appropriate adjustment for our discrete distribution. Since the Poisson is a non-negative integer distribution, we know that $Pr\{a-1 < X < a\} = 0$, and therefore $Pr\{X < a\} = Pr\{X \leq a-1\} = F(a-1)$. Now, with this understanding, we would adjust the general forms for our sample problem (and for any integer distribution) as follows:

$$F_{a,b}(x) = \frac{F(x)-F(a-1)}{F(b)-F(a-1)}$$
$$x = F_{a,b}^{-1}(\alpha)=F^{-1}\big(\alpha(x)\big[F(b)-F(a-1)\big] + F(a-1)\big).$$


### Truncated probability mass and density functions

As you might guess, the forms for the truncated probability mass $m(x)$ and density $f(x)$ functions are, respectively:

$$m_{a,b}(x) = \frac{m(x)}{F(b) - F(a)}$$
